...
template <typename T>
float can_get_signal_raw(const uint8_t* buf, const size_t startBit, const size_t length, const bool isIntel, const float factor, const float offset)
{
...
template <typename T>
constexpr void can_set_signal_raw(uint8_t* buf, const float val, const size_t startBit, const size_t length, const bool isIntel,
